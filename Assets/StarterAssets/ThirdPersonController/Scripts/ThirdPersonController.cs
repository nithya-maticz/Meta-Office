...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour, IPunObservable
    {
        ...
        private void Awake()
        {
            ...
            playerCamera.SetActive(PV.IsMine);
            cinemachine.SetActive(PV.IsMine);
        }
        ...
        public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
        {
            ...
            else
            {
                ...
                float moveY = (float)stream.ReceiveNext();
                Grounded = (bool)stream.ReceiveNext();

                if (_hasAnimator && !PV.IsMine)
                {
                    _animator.SetFloat(_animIDSpeed, _animationBlend);
                    _animator.SetFloat(_animIDMotionSpeed, new Vector2(moveX, moveY).magnitude);
                    _animator.SetBool(_animIDGrounded, Grounded);
                }
            }
        }
